...
describe('ContenidoDigitalService', () => {
  ...
  describe('crearImagen', () => {
    test('debe crear una nueva imagen correctamente', () => {
      const nuevaImagen = {
        productoId: 1,
      ...
      const resultado = service.crearImagen(nuevaImagen);
  ...
  describe('obtenerImagenesPorEtiqueta', () => {
    test('debe filtrar imágenes por etiqueta', () => {
      service.crearImagen({
        productoId: 1,
        url: 'img1.jpg',
        titulo: 'Imagen 1',
      ...
      const resultado = service.obtenerImagenesPorEtiqueta('chocolate');
  ...
  describe('agregarEtiqueta', () => {
    test('debe agregar una etiqueta a una imagen existente', async () => {
      const imagen = service.crearImagen({
        productoId: 1,
        url: 'img.jpg',
        titulo: 'Test',
        etiquetas: ['original'],
  ...
  describe('eliminarImagen', () => {
    test('debe eliminar una imagen correctamente', () => {
      const imagen = service.crearImagen({
        productoId: 1,
        url: 'img.jpg',
        titulo: 'Test',
      ...
      service.eliminarImagen(imagen.id);
      const todas = service.obtenerTodasLasImagenes();
