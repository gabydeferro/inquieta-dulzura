...
export class AuthService {
  ...
  async login(credentials: LoginCredentials): Promise<{
    ...
    try {
      ...
      const passwordMatch = await bcrypt.compare(credentials.password, usuario.password_hash);
  ...
  async refreshAccessToken(refreshToken: string): Promise<{
    ...
    try {
      ...
      const tokenData = tokens[0] as { id: number; email: string; rol: 'admin' | 'usuario' };
