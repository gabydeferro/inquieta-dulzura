...
export class FotoService {
  ...
  private static readonly TIPOS_PERMITIDOS = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
  ...
  constructor() {
    ...
    if (!this.usarCloudinary) {
      void this.inicializarDirectorio();
    }
  }
  ...
  async subirFoto(
    params: SubirFotoParams,
  ): Promise<{ success: boolean; message: string; data?: Foto }> {
    const { producto_id, archivo } = params;
  ...
  async obtenerFotosProducto(producto_id: number): Promise<Foto[]> {
    const query =
      'SELECT * FROM fotos_productos WHERE producto_id = ? ORDER BY es_principal DESC, orden ASC';
  ...
  async obtenerFotoPorId(id: number): Promise<Foto> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      'SELECT * FROM fotos_productos WHERE id = ?',
      [id],
    );
  ...
  async obtenerFotoPrincipal(producto_id: number): Promise<Foto | null> {
    const query =
      'SELECT * FROM fotos_productos WHERE producto_id = ? AND es_principal = TRUE LIMIT 1';
    const [rows] = await pool.execute<RowDataPacket[]>(query, [producto_id]);
  ...
  async limpiarArchivosHuerfanos(): Promise<{
    success: boolean;
    message: string;
    eliminados: number;
  }> {
    if (this.usarCloudinary)
      return { success: false, message: 'No disponible en Cloudinary', eliminados: 0 };
    try {
      const archivos = await fs.readdir(this.directorioBase);
      const [rows] = await pool.execute<RowDataPacket[]>(
        'SELECT ruta_completa FROM fotos_productos WHERE ruta_completa IS NOT NULL',
      );
      const rutasEnBD = new Set(rows.map((r) => (r as { ruta_completa: string }).ruta_completa));
      let eliminados = 0;
  ...
  private async validarProductoExiste(producto_id: number): Promise<void> {
    const [rows] = await pool.execute<RowDataPacket[]>('SELECT id FROM productos WHERE id = ?', [
      producto_id,
    ]);
  ...
  private async desmarcarPrincipal(producto_id: number): Promise<void> {
    await pool.execute('UPDATE fotos_productos SET es_principal = FALSE WHERE producto_id = ?', [
      producto_id,
    ]);
  }
  ...
  private async obtenerSiguienteOrden(producto_id: number): Promise<number> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      'SELECT COALESCE(MAX(orden), 0) + 1 as sig FROM fotos_productos WHERE producto_id = ?',
      [producto_id],
    );
