...
export class RecetaService {
  ...
  async getById(id: number): Promise<RecetaDTO | null> {
    ...
    receta.ingredientes = ingredientes.map((row) => ({
      ingrediente_id: row.ingrediente_id as number,
      cantidad: row.cantidad as number,
      unidad_medida: row.unidad_medida as string,
      notas: row.notas as string | undefined,
      ingrediente: {
        id: row.ingrediente_id as number,
        nombre: row.nombre as string,
        descripcion: row.descripcion as string,
        unidad_medida: row.unidad_medida as string,
        costo_unitario: row.costo_unitario as number,
        activo: true,
      },
  ...
  async create(data: CreateRecetaDTO): Promise<RecetaDTO> {
    ...
    try {
      ...
      const { nombre, descripcion, instrucciones, tiempo_preparacion, porciones, ingredientes } =
        data;
